...
World buildScene2() {
    ...
    world.setLight(Light(Point(-7, 12, -10), Color(0.8, 0.7, 1.0)));

    Sphere floor;
    floor.setTransform(Matrix::scaling(10, 0.01, 10));
    floor.getMaterial().setSurfaceColor(Color(0.95, 0.95, 1.0));
    floor.getMaterial().setSpecular(0);
    world.getSpheres().push_back(floor);
    ...
    Sphere middle;
    middle.setTransform(Matrix::translation(-0.5, 1, 0.5));
    middle.getMaterial().setSurfaceColor(Color(0.6, 0.5, 0.9));
    middle.getMaterial().setDiffuse(0.7);
    middle.getMaterial().setSpecular(0.4);
    world.getSpheres().push_back(middle);

    Sphere right;
    right.setTransform(Matrix::translation(1.5, 0.5, -0.5) *
                       Matrix::scaling(0.5, 0.5, 0.5));
    right.getMaterial().setSurfaceColor(Color(0.9, 0.8, 1.0));
    right.getMaterial().setDiffuse(0.7);
    right.getMaterial().setSpecular(0.3);
    world.getSpheres().push_back(right);
    ...
    left.setTransform(Matrix::translation(-1.5, 0.33, -0.75) *
                      Matrix::scaling(0.33, 0.33, 0.33));
    left.getMaterial().setSurfaceColor(Color(0.5, 0.7, 0.8));
    left.getMaterial().setDiffuse(0.7);
    left.getMaterial().setSpecular(0.3);
    world.getSpheres().push_back(left);
...
int main() {
    ...
    // chapter5();
    // chapter6();
    const World w = buildScene2();
    // Camera camera(200, 100, M_PI / 3);
    Camera camera(1920, 1080, M_PI / 3);
    camera.setTransform(Matrix::viewTransform(Point(3.5, 2, -5), Point(0, 1, 0), Vector(0, 1, 0)));
